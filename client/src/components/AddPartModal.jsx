...
import { X, Package, Plus, AlertCircle, MapPin } from 'lucide-react';
...
const AddPartModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    try {
      ...
      if (res.data && res.data.success) {
        ...
        if (res.data.data.length > 0) {
          setFormData(prev => ({ ...prev, branch_id: prev.branch_id || res.data.data[0].id }));
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await inventoryService.createPart(formData);
      if (res.success) {
      ...
      setError(err.response?.data?.message || 'Gagal mendaftarkan spare part baru.');
    } finally {
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 overflow-hidden my-8">
        ...
        <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-bold text-base">Tambah Spare Part Baru</h3>
              <p className="text-xs text-slate-400">Pendaftaran SKU Katalog Spare Part Per Lokasi Cabang</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="Contoh: RAM-DDR5-32G"
            ...
            <div className="sm:col-span-2">
              ...
              <input
                ...
                value={formData.name}
                onChange={handleChange}
                placeholder="Contoh: Kingston DDR5 32GB 5600MHz SODIMM"
                className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-cyan-500 focus:outline-none"
                required
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                min="0"
                className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-cyan-500 focus:outline-none font-semibold"
              />
            </div>
            ...
            <div className="sm:col-span-2">
              ...
              <input
                ...
                min="0"
                step="1000"
                className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-cyan-500 focus:outline-none font-mono"
              />
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 flex items-center justify-end space-x-3">
            ...
            <button
              ...
              className="px-5 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white text-xs font-semibold rounded-xl shadow-md shadow-cyan-600/20 disabled:opacity-50 flex items-center space-x-1.5"
            >
              {isSubmitting ? (
                <span>Memproses...</span>
              ) : (
                <>
                  ...
                  <span>Daftarkan Spare Part</span>
                </>
              )}
            </button>
          </div>
